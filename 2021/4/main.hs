...
part2 (i:numbers) boards latest    = do
    ...
    part2' winners i numbers (filter (`notElem` winners) run) latest
...
split' :: [String] -> [[String]]
split' [] = []
split' b  = concat [filter (/="") $ splitOn " " k | k <- take 5 b] : split' (drop 5 b)
...
sumUnmarked b = sum $ map num $ filter check' b
